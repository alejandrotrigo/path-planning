      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            Vehicle ego = Vehicle(2048);
            // GET ALL THE CARS ARROUND US THAT ARE VISIBLE
            std::map<int, Vehicle*> vehicles;
            for(auto data: sensor_fusion)
            {
              Vehicle *vehicle = new Vehicle(data[0], data[1], data[2], data[3], data[4], data[5], data[6]);
              if ((*vehicle).is_visible()){
                if(vehicles.find((*vehicle).id) == vehicles.end()){
                  vehicles[data[0]] = vehicle;
                }else{
                  auto it = vehicles.find(data[0]);
                  vehicles.erase((int)(*vehicle).id);
                  vehicles[data[0]] = vehicle;
                }
              }else{
                auto it = vehicles.find((*vehicle).id);
                if(it != vehicles.end()){
                  vehicles.erase((int) (*vehicle).id);
                }
              }
            ...
            //vector<Vehicle> vehicles = ego.generate_predictions(50);
            std::map<int, vector<Vehicle>> predictions;
            

            std::cout << "lane: " << lane << " speed: " << car_speed << " there is a car behind?: " << std::endl;
            ...
            for (unsigned int i = 0; i < sensor_fusion.size(); i++)
            {
              ...
              if (check_lane == lane)
              {
                ...
                {
                  //TODO Do some logic here, lower reference velocity so we don't crash into the car in front of us, could
